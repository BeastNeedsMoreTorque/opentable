...
const GallagherIndexCalculator = () => {
    ...
    /* Alternative 2 separate handler functions.
    
    const handleStringInputChange = (index: number, field: 'party_name', value: string) => {
      const newPartyData = [...partyData];
      newPartyData[index][field] = value;
      setPartyData(newPartyData);
    };
  
  const handleNumberInputChange = (index: number, field: 'votes_pct' | 'seats_pct', value: number) => {
    const newPartyData = [...partyData];
    newPartyData[index][field] = value;
    setPartyData(newPartyData);
  };
  
  //then in JSX use appropriate handler for each input:
  <input
    type="text"
    value={party.party_name}
    onChange={(e) => handleStringInputChange(index, 'party_name', e.target.value)}
  />
  <input
    type="number"
    value={party.votes_pct}
    onChange={(e) => handleNumberInputChange(index, 'votes_pct', Number(e.target.value))}
  />
  <input
    type="number"
    value={party.seats_pct}
    onChange={(e) => handleNumberInputChange(index, 'seats_pct', Number(e.target.value))}
  />
